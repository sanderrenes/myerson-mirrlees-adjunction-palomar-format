...
It contains the main theorem statement: the adjunction T ⊣ Q that unifies
Myerson's Revenue Equivalence Theorem and the Mirrlees Taxation Principle.

## Main Result

* `MechDesign.adj_T_Q` — The adjunction T ⊣ Q on the regular subcategories
  `AllocR`/`MechR` (Theorem 4.3).  The restriction to regular objects is not a
  convenience: on all of **Mech** the hypotheses the envelope theorem needs at every
  object cannot be satisfied, and the statement would be vacuous.  See
  `MechDesign.Corollaries.no_uniform_lipschitz` and
  `MechDesign.Corollaries.no_uniform_hW`.
